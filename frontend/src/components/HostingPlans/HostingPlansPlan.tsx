...
import {
	computeSavePercentTwoYearPlan,
	computeSavePercentYearPlan,
	convertTwoYearlyToMonthly,
...
const HostingPlansPlan = ({ plan, maxLinesNumber }: HostingPlansPlanProps) => {
	...
	const addingLinesArray = Array(addingLinesNumber).fill(0);

	for (let i = 0; i < addingLinesNumber; i++) {
		addingLinesArray[i] = i;
	}
		...
		<Paper elevation={3} sx={{ p: 2 }}>
					...
					<Chip
						label={`Save ${
							selectedDuration === "month"
								...
								? computeSavePercentYearPlan(plan)
								: computeSavePercentTwoYearPlan(plan)
						}%`}
					/>
				</Box>
			...
			{plan.features.map((feature) => (
				...
				>
					<Typography
						...
						sx={{
							display: feature.note !== undefined ? "inline-block" : undefined,
						}}
						gutterBottom
					>
						+ {feature.content}
			...
			{addingLinesArray.map((i) => (
				...
				>
					<Typography
						gutterBottom
						sx={{ userSelect: "none", color: "transparent" }}
					>
						.
					</Typography>
				</Box>
			...
			<Button variant="outlined" component="a" href="#plans-details" fullWidth sx={{ mt: 2 }}>
				Compare Plans
			</Button>
		</Paper>
	);
